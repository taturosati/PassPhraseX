...
pub fn write_private_key(sk: Vec<u8>) -> anyhow::Result<()> {
    write_bytes(PRIVATE_KEY_FILE, sk)
}

pub fn write_signing_key(sk: Vec<u8>) -> anyhow::Result<()> {
    write_bytes(SIGNING_KEY_FILE, sk)
}

pub fn read_private_key() -> anyhow::Result<Vec<u8>> {
    read_bytes(PRIVATE_KEY_FILE)
}

pub fn read_signing_key() -> anyhow::Result<Vec<u8>> {
    read_bytes(SIGNING_KEY_FILE)
}
